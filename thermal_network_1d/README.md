...
ブラウザだけで動くツールを 3 つ同梱しています。サーバも外部ライブラリも不要です。

```bash
xdg-open gui/heat_path.html                # 材質と寸法だけ入れる (macOS なら open)
xdg-open gui/thermal_bench.html            # ノードと熱抵抗で組む
xdg-open gui/thermal_circuit_studio.html   # 相関式・過渡つきの全部入り
```

### 熱の通り道 (`heat_path.html`) --- 熱抵抗を知らなくても設計できる

部品を上から積むだけです。入力するのは **材質・寸法・発熱量・風速・周囲温度** のみ。

- 厚み方向の伝導 `L/(kA)`、**熱の広がり(拡がり抵抗、Lee et al.)**、TIM、接触熱抵抗、
  自然対流(Churchill–Chu / フィン間は Elenbaas)、強制対流、フィン効率、
  **輻射**(対流と並列)、**流体そのものの温度上昇** `1/(2 m' cp)` を自動で当てはめます
- 表面温度に依存する項があるので収束するまで反復します
- **許容温度に対する余裕**を判定し、「フィンを 1.5 倍に」「高性能グリスに」などを
  **実際に解き直して効き目(K)の大きい順**に並べます
- どの層のどの物理で温度を捨てているかの内訳と、h・Re・Ra・Bi・フィン効率まで表示します

1D 化で忘れられやすい**拡がり抵抗・輻射・流体の昇温**が、意識しなくても必ず入るのが狙いです。

### 熱回路ベンチ (`thermal_bench.html`) --- 要素は 4 つだけ

- **ノード**(温度が決まる点) / **枝**(熱抵抗 R [K/W]) / **発熱**(Q [W]) / **温度固定**([degC])
- ノードを置き、右上の ＋ を相手にドラッグしてつなぎ、R と Q を打つと**その場で解けます**
- ノードの色 = 温度、枝の太さ = 熱流の大きさ、流れる破線と矢印 = 熱の向き
- 右パネルに温度・熱流・検算(収支の残差)。枝には `L/(kA)`、`1/(hA)`、`1/(mdot cp)` の電卓つき

### 熱回路スタジオ (`thermal_circuit_studio.html`) --- 過渡・非線形まで

- 伝導 / 界面(TIM) / 対流 / 輻射 / フィン / 拡がり抵抗 / 流体への排熱を選んでつなぎます
- 対流・輻射は相関式を選ぶと熱伝達率が温度の関数になり、非線形反復で解きます
- 過渡モードでは熱容量を含めて時間積分し、再生ボタンで温度が動く様子を見られます

3 つとも組んだ内容を **YAML に書き出して `python -m thermalnet.model` / `report` でそのまま解けます**。
計算式は Python 版と同一です。詳しくは `gui/README.md`。
