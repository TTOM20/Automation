...
ブラウザだけで動く**熱回路シミュレータ**を同梱しています。サーバも外部ライブラリも不要です。

```bash
xdg-open gui/thermal_circuit_studio.html      # macOS なら open
```

- ノードを置き、パレットから要素(伝導・対流・輻射・フィン・拡がり抵抗・流体への排熱…)を
  選んでクリックでつなぐと、**その場で温度と熱流が計算**されます
- ノードの色 = 温度、枝の太さ = 熱流の大きさ、流れる破線 = 熱の向き
- 過渡モードでは熱容量を含めて時間積分し、再生ボタンで温度が動く様子を見られます
- 組んだ回路は **YAML に書き出して `python -m thermalnet.model` / `report` でそのまま解けます**
- 例題(空冷 CPU / 多層壁と熱橋 / ファンレス筐体 / パワーモジュール水冷)を 1 クリックで読み込み
